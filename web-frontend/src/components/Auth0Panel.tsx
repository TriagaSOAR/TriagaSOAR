...
const LOG_TYPE_LABELS: Record<string, string> = {
  ...
  fp: "Brute Force",
  fu: "Failed User",
  fco: "Failed Connector",
  limit_wc: "Anomaly",
  cls: "Credential Stuffing",
  slo: "Logout",
  ss: "Success Signup",
...
export default function Auth0Panel() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/auth0/users/${encodeURIComponent(selectedUser)}`)
      .then(r => r.json())
      .then(setUserDetail);
  }, [selectedUser]);
  ...
  if (!health?.available) return (
    <div style={{ color: "#ff8c42", fontFamily: "monospace", padding: 32 }}>
      {health?.message ?? "Auth0 not configured. Set AUTH0_DOMAIN, AUTH0_CLIENT_ID, AUTH0_CLIENT_SECRET in .env."}
    </div>
  );
          ...
          <div>
            ...
            {users.map(u => (
              <div
                ...
                style={{
                  background: "#13131e",
                  border: `1px solid ${selectedUser === u.user_id ? "#7b61ff" : "#2a2a3a"}`,
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>
                    {u.identities?.join(", ")} · {u.logins_count} logins
                  </div>
                </div>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                  {u.blocked
                    ? <Badge label="Blocked" color="#ff4d6a" />
                    : <Badge label="Active" color="#06d6a0" />
                  }
                ...
                <>
                  ...
                  {(userDetail.recent_apps?.length ?? 0) > 0 && (
                    <div style={{ marginBottom: 12 }}>
                      ...
                      {userDetail.recent_apps?.map(a => (
                        <div key={a} style={{ fontSize: 12, color: "#c0c0d0", padding: "2px 0" }}>{a}</div>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    {!userDetail.blocked ? (
                      <button
                        onClick={() => doAction("block", selectedUser)}
                      ...
                      <button
                        onClick={() => doAction("unblock", selectedUser)}
                        disabled={actionLoading}
                        style={{
          ...
          {logs.map(l => (
            <div key={l._id} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, color: "#e0e0f0", marginBottom: 4 }}>
                    {l.description || LOG_TYPE_LABELS[l.type] || l.type}
                  </div>
                  ...
                  <div style={{ fontSize: 10, color: "#555", fontFamily: "monospace", marginTop: 4 }}>
                    {l.type} · {l.date ? new Date(l.date).toLocaleString() : ""}
                  </div>
                </div>
                <Badge
                  label={LOG_TYPE_LABELS[l.type] || l.type}
                  color={LOG_TYPE_COLORS[l.type] || "#888"}
                />
              </div>
            </div>
          ))}
            ...
            <div key={key} style={{ background: "#13131e", border: "1px solid #2a2a3a", borderRadius: 8, padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                {data?.enabled !== undefined && (
                  <Badge label={data.enabled ? "Enabled" : "Disabled"} color={data.enabled ? "#06d6a0" : "#ff4d6a"} />
                )}
              </div>
              <pre style={{
