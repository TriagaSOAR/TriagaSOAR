...
export default function OktaPanel() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/okta/users/${selectedUser}`)
      .then(r => r.json())
      .then(setUserDetail);
  }, [selectedUser]);
  ...
  if (!health?.available) return (
    <div style={{ color: "#ff8c42", fontFamily: "monospace", padding: 32 }}>
      {health?.message ?? "Okta not configured. Set OKTA_DOMAIN and OKTA_API_TOKEN in .env."}
    </div>
  );
    ...
    <div style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: "#888" }}>
          Connected to <span style={{ color: "#7b61ff", fontFamily: "monospace" }}>{health.domain}</span>
        </div>
          ...
          <div>
            ...
            {users.map(u => (
              <div
                ...
                style={{
                  background: "#13131e",
                  border: `1px solid ${selectedUser === u.id ? "#7b61ff" : "#2a2a3a"}`,
                ...
                <>
                  ...
                  <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <div style={{ background: "#0d0d14", borderRadius: 6, padding: 10 }}>
                      ...
                      <div style={{ fontSize: 20, fontWeight: 700, color: "#e0e0f0" }}>{userDetail.signin_count_48h}</div>
                    </div>
                    <div style={{ background: "#0d0d14", borderRadius: 6, padding: 10 }}>
                      <div style={{ fontSize: 10, color: "#888", fontFamily: "monospace", marginBottom: 4 }}>FAILURES 48H</div>
                      <div style={{ fontSize: 20, fontWeight: 700, color: (userDetail.failed_signin_count_48h ?? 0) > 0 ? "#ff4d6a" : "#e0e0f0" }}>
                        {userDetail.failed_signin_count_48h}
                      </div>
                    </div>
                  </div>
                  ...
                  {(userDetail.recent_ips?.length ?? 0) > 0 && (
                    <div style={{ marginTop: 12 }}>
                      ...
                      {userDetail.recent_ips?.map(ip => (
                        <div key={ip} style={{ fontSize: 12, fontFamily: "monospace", color: "#c0c0d0", padding: "2px 0" }}>{ip}</div>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    <button
                      onClick={() => doAction("clear-sessions", selectedUser)}
                      disabled={actionLoading}
                      style={{
                        padding: "8px 12px", borderRadius: 6, border: "1px solid #ffd16644",
                        background: "#ffd16611", color: "#ffd166", cursor: "pointer", fontSize: 12, fontWeight: 600,
                      }}
                    >
                      Clear Sessions
                    </button>
                    {userDetail.status === "ACTIVE" && (
                      <button
                        onClick={() => doAction("suspend", selectedUser)}
                        disabled={actionLoading}
                        style={{
                          padding: "8px 12px", borderRadius: 6, border: "1px solid #ff4d6a44",
                          background: "#ff4d6a11", color: "#ff4d6a", cursor: "pointer", fontSize: 12, fontWeight: 600,
                        }}
                      >
                        Suspend User
                      </button>
                    ...
                    {userDetail.status === "SUSPENDED" && (
                      <button
                        onClick={() => doAction("unsuspend", selectedUser)}
                        disabled={actionLoading}
                        style={{
                          padding: "8px 12px", borderRadius: 6, border: "1px solid #06d6a044",
                          background: "#06d6a011", color: "#06d6a0", cursor: "pointer", fontSize: 12, fontWeight: 600,
                        }}
                      >
                        Unsuspend User
                      </button>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#888", fontFamily: "monospace" }}>
                    {l.actor?.displayName}
                    {l.actor?.alternateId ? ` (${l.actor.alternateId})` : ""}
                    {l.client?.ipAddress ? ` · ${l.client.ipAddress}` : ""}
                    {l.client?.geographicalContext?.city ? ` · ${l.client.geographicalContext.city}, ${l.client.geographicalContext.country}` : ""}
                  </div>
                  <div style={{ fontSize: 10, color: "#555", fontFamily: "monospace", marginTop: 4 }}>
                    {l.eventType} · {l.published ? new Date(l.published).toLocaleString() : ""}
                  </div>
                </div>
                {l.outcome?.result && (
                  <Badge label={l.outcome.result} color={OUTCOME_COLORS[l.outcome.result] ?? "#888"} />
                )}
              </div>
            </div>
          ))}
